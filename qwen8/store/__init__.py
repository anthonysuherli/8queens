...
Backend selection (in order):
  1. ``QWEN8_BACKEND`` env var — ``"local"`` or ``"cloud"`` (explicit override);
...
calls; a different ``QWEN8_DB_PATH`` — as tests use — gets its own store). The
...
``[cloud]`` extra nor a populated ``qwen8.core.config`` — imports this module
cleanly. The creds sniff is likewise resilient: a missing ``qwen8.core.config``
...
from qwen8.store.base import Store
from qwen8.store.sqlite import SQLiteStore, _default_db_path
...
def _has_cloud_creds() -> bool:
    ...
    Resilient by design: ``qwen8.core.config`` is provided by a later task, so
    ...
    tier must work without ``qwen8.core.config`` present.

    Note: when present, `get_settings()` is `@lru_cache`d, so creds are sniffed
    once per process. Set ``QWEN8_BACKEND`` explicitly to skip sniffing.
    """
    try:
...
def active_backend() -> str:
    ...
    fork (local skips GoTrue login). ``QWEN8_BACKEND`` overrides creds-sniffing.
    """
    backend = os.getenv("QWEN8_BACKEND")
    if not backend:
...
def get_store(
    ...
    *,
    ...
    if active_backend() == "local" or db_path is not None:
        ...
        return store
    from qwen8.store.supabase import SupabaseStore
