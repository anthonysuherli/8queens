...
Environment overrides use the ``QWEN8_<SECTION>__<FIELD>`` form, e.g.
``QWEN8_AGENT__TEMPERATURE=0.4`` or ``QWEN8_SEARCH__MIN_SIMILARITY=0.2``.

The YAML file location defaults to the repo-root ``config.yaml`` and can be
pointed elsewhere with the ``QWEN8_CONFIG_FILE`` environment variable.
"""
...
class Settings(BaseSettings):
    ...
    # Backend selection (the store seam). ``QWEN8_BACKEND`` forces "local" or
    # "cloud"; ``QWEN8_DB_PATH`` points the local SQLite store at a custom file.
...
You are Qwen8 — an agent that helps the user grow a knowledge base.
...
def _config_path() -> Path:
    override = os.getenv("QWEN8_CONFIG_FILE")
    if override:
        return Path(override)
    # config.py lives at <root>/qwen8/core/config.py → <root>/config.yaml
...
def _env_overrides() -> dict:
    """Collect `QWEN8_<SECTION>__<FIELD>` env vars into a nested dict.
