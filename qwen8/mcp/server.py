"""FastMCP stdio server — the open-core plugin's entry path into qwen8.
...
deliberately small — four tools:
    ...
    qwen8_resume    — inject KB context (banner + preamble + coverage)
    qwen8_search    — semantic search over existing findings
    qwen8_explore   — run the research pipeline + persist findings
    qwen8_projects  — list the caller's projects/KBs

Run with: ``python -m qwen8.mcp.server``.
"""
...
from qwen8.core.agent.preamble import Depth, select_preamble
...
from qwen8.core.exploration import run_exploration
from qwen8.core.knowledge_graph.builder import schedule_kg_update
from qwen8.store import get_store

from .banner import QWEN8_BANNER
...
async def qwen8_resume(
    ...
    "coverage"}``: the qwen8 wordmark to lead the message with, the <preamble>
    ...
    return {"banner": QWEN8_BANNER, "preamble": preamble, "coverage": coverage}
...
@mcp.tool()
async def qwen8_search(project: str, kb: str, query: str, limit: int | None = None) -> dict:
