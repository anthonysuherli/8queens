...
Resolution is by *name* for qwen8-style ergonomics; missing project/KB are
...
def _service_client():
    ...
    from qwen8.core.clients.supabase import service_client  # lazy: cloud-only
...
def resolve_store():
    ...
    in for an RLS-scoped token. Used by the cross-repo ``qwen8_projects`` tool.
    """
    from qwen8.store import active_backend, get_store
