"""FastAPI app for the open-core engine — the loopback HTTP surface.
    ...
    uvicorn queens8.api.main:app ──► GET  /health
                                    GET  /api/projects
...
from queens8.api.health import router as health_router
from queens8.api.routes_explore import router as explore_router
from queens8.api.routes_society import router as society_router
from queens8.api.routes_findings import router as findings_router
from queens8.api.routes_kg import router as kg_router
from queens8.api.routes_projects import router as projects_router
from queens8.core.config import _ENV_PREFIX, get_settings
...
def _assert_startup_invariants() -> None:
    ...
    assert _ENV_PREFIX == "QUEENS8_", f"env prefix not renamed: {_ENV_PREFIX!r}"
    s = get_settings()
    assert s.tavily_api_key, "TAVILY_API_KEY is required (search_mode=tavily)"
    # Resolve the running store and prove it is the queens8 brain, not delapan's.
    store = get_store(None, org_id="local")
    db_path = str(getattr(store, "db_path", ""))
    assert ".queens8" in db_path, f"store db_path not the queens8 brain: {db_path!r}"
