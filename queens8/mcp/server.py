"""FastMCP stdio server — the open-core plugin's entry path into queens8.
...
deliberately small — four tools:
    ...
    queens8_resume    — inject KB context (banner + preamble + coverage)
    queens8_search    — semantic search over existing findings
    queens8_explore   — run the research pipeline + persist findings
    queens8_projects  — list the caller's projects/KBs

Run with: ``python -m queens8.mcp.server``.
"""
...
from queens8.core.agent.preamble import Depth, select_preamble
from queens8.core.agent.synopsis import maybe_rebuild_synopsis
from queens8.core.clients.embeddings import embed_batch, embed_text
from queens8.core.config import get_config, get_settings
from queens8.core.exploration import run_exploration
from queens8.core.knowledge_graph.builder import schedule_kg_update
from queens8.store import get_store

from .banner import QUEENS8_BANNER
...
async def queens8_resume(
    ...
    "coverage"}``: the queens8 wordmark to lead the message with, the <preamble>
    ...
    return {"banner": QUEENS8_BANNER, "preamble": preamble, "coverage": coverage}
...
@mcp.tool()
async def queens8_search(project: str, kb: str, query: str, limit: int | None = None) -> dict:
...
@mcp.tool()
async def queens8_projects() -> dict:
