...
Resolution is by *name* for queens8-style ergonomics; missing project/KB are
...
def _service_client():
    ...
    from queens8.core.clients.supabase import service_client  # lazy: cloud-only
...
def resolve_tenant(project: str, kb: str, *, create: bool = True) -> TenantContext:
    ...
    """
    from queens8.store import active_backend, get_store
...
def resolve_store():
    ...
    in for an RLS-scoped token. Used by the cross-repo ``queens8_projects`` tool.
    """
    from queens8.store import active_backend, get_store
