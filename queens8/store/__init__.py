...
Backend selection (in order):
  1. ``QUEENS8_BACKEND`` env var — ``"local"`` or ``"cloud"`` (explicit override);
...
calls; a different ``QUEENS8_DB_PATH`` — as tests use — gets its own store). The
...
``[cloud]`` extra nor a populated ``queens8.core.config`` — imports this module
cleanly. The creds sniff is likewise resilient: a missing ``queens8.core.config``
...
from queens8.store.base import Store
from queens8.store.sqlite import SQLiteStore, _default_db_path
...
def _has_cloud_creds() -> bool:
    ...
    Resilient by design: ``queens8.core.config`` is provided by a later task, so
    ...
    tier must work without ``queens8.core.config`` present.

    Note: when present, `get_settings()` is `@lru_cache`d, so creds are sniffed
    once per process. Set ``QUEENS8_BACKEND`` explicitly to skip sniffing.
    """
    try:
...
def active_backend() -> str:
    ...
    fork (local skips GoTrue login). ``QUEENS8_BACKEND`` overrides creds-sniffing.
    """
    backend = os.getenv("QUEENS8_BACKEND")
    if not backend:
...
def get_store(
    ...
    *,
    ...
    if active_backend() == "local" or db_path is not None:
        ...
        return store
    from queens8.store.supabase import SupabaseStore
