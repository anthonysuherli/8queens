...
def _default_db_path() -> str:
    """``QUEENS8_DB_PATH`` if set, else ``~/.queens8/queens8.db`` (dir created)."""
    env = os.environ.get("QUEENS8_DB_PATH")
    if env:
        return env
    home = Path.home() / ".queens8"
    home.mkdir(parents=True, exist_ok=True)
    return str(home / "queens8.db")
