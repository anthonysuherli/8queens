...
from queens8.society.blackboard import claim_gap, complete_gap, create_gaps
...
def _make_store():
    ...
    d = tempfile.mkdtemp()
    path = os.path.join(d, ".queens8", "queens8.db")
...
def _make_app(monkeypatch, store):
    ...
    monkeypatch.setenv("QUEENS8_SOCIETY_SECRET", _SECRET)
    ...
    monkeypatch.setattr("queens8.api.main._assert_startup_invariants", lambda: None)

    # Reload routes_society so _RUNS starts fresh and settings are re-read.
    sys.modules.pop("queens8.api.routes_society", None)
    import queens8.api.routes_society as rs
    rs._RUNS.clear()
    ...
    app = FastAPI(title="queens8-test")
...
def _make_mock_run_society(kb_id: str):
    ...
    async def _fake(topic, *, org_id, project_id, kb_id, cfg, n_researchers=2,
        ...
        from queens8.society.loop import SocietyResult
...
def test_start_and_stream_with_correct_secret(monkeypatch):
    ...
    monkeypatch.setattr("queens8.api.routes_society.run_society", _make_mock_run_society(ctx.kb_id))
